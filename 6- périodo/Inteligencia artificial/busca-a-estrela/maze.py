...
Pra trocar de algoritmo (aleatória/DFS/BFS/A*), use as teclas 1, 2, 3 e 4
durante a execução — quem faz a busca é sempre busca(), em busca.py.
...
Controles:
  1 / 2 / 3 / 4 = aleatória / DFS / BFS / A*  (roda de novo já no novo algoritmo)
  R = reinicia a busca
...
from busca import busca, ALGORITMOS
...
historico = []       # lista de estados devolvida por busca.py: um item por passo
indice = 0           # em qual item do histórico estamos agora
algoritmo = "astar"  # algoritmo ativo; troca pelas teclas 1/2/3/4

NOMES = {
    "aleatoria": "Aleatoria [1]",
    "dfs": "DFS [2]",
    "bfs": "BFS [3]",
    "astar": "A* (g+h) [4]",
}
TECLAS = {}  # preenchido em main(), depois que o pygame existe


def reset():
    """Roda a busca de novo do zero, com o algoritmo selecionado agora."""
    global historico, indice
    historico = busca(INICIO, OBJETIVO, vizinhos, algoritmo)
    indice = 0
...
def desenhar(screen, fonte, fps):
    ...
    linha1 = f"Algoritmo: {NOMES[algoritmo]}   |   {status}"
    linha2 = f"Passos: {passos}   Visitados: {len(visitados)}   Fronteira: {len(fronteira)}   Velocidade: {fps}/s"
    linha3 = "1-4 troca de algoritmo   |   R reinicia   |   +/- velocidade   |   ESC sai"
...
def main():
    ...
    # teclas 1..4 -> nomes dos algoritmos, na ordem de ALGORITMOS (busca.py)
    TECLAS.update({
        pygame.K_1: ALGORITMOS[0], pygame.K_KP1: ALGORITMOS[0],
        pygame.K_2: ALGORITMOS[1], pygame.K_KP2: ALGORITMOS[1],
        pygame.K_3: ALGORITMOS[2], pygame.K_KP3: ALGORITMOS[2],
        pygame.K_4: ALGORITMOS[3], pygame.K_KP4: ALGORITMOS[3],
    })
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                if e.key == pygame.K_ESCAPE:
                    ...
                    reset()
                elif e.key in TECLAS:
                    algoritmo = TECLAS[e.key]  # 1/2/3/4: troca e roda de novo
                    reset()
